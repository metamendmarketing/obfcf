...
import { ChevronLeft, ChevronRight, X, Play, SkipBack, SkipForward } from "lucide-react";
...
export default function PresentationPage() {
  ...
  useEffect(() => {
    ...
    // Slight delay to allow LivePreview to render all sections
    const timeoutId = setTimeout(updateMetrics, 500);
    window.addEventListener('resize', updateMetrics);
    
    // Aggressive polling to catch Chromium multi-column layout expansions that occur after network images load
  ...
  useEffect(() => {
    ...
    const handleWheel = (e: WheelEvent) => {
      ...
      const now = Date.now();
      if (now - lastWheelTime < 400) return; // 400ms cooldown to prevent aggressive scrolling
      
      // Support both vertical (mouse wheel) and horizontal (trackpad swipe) scrolling
    ...
    const handleFullscreenChange = () => {
      // If user pressed ESC and exited fullscreen natively, route them out.
  ...
  }, [hasStarted, lightboxSrc, totalPages, router]);
      ...
      <div className="min-h-screen bg-[#050505] flex flex-col items-center justify-center text-white p-6">
        <div className="bg-[#0A0A0A] p-12 rounded-2xl border border-white/10 shadow-2xl flex flex-col items-center max-w-lg text-center animate-in fade-in zoom-in duration-500">
          ...
          <p className="text-white/60 mb-8">
            You are about to enter full-screen presentation mode for <span className="text-white font-medium">{audit.companyName}</span>. All editing controls have been locked.
          </p>
          <div className="flex gap-4 w-full">
            <button 
              onClick={() => router.push('/dashboard')}
              className="flex-1 py-3 px-6 rounded-full font-medium bg-white/5 hover:bg-white/10 transition-colors"
            >
              Cancel
            </button>
            <button 
    ...
    <div className="fixed inset-0 z-[100] bg-[#050505] overflow-hidden">
      ...
      {currentPage < totalPages - 1 && (
        <button 
          onClick={handleNext}
          className="absolute top-1/2 -translate-y-1/2 w-16 h-16 bg-black/50 text-white rounded-full flex items-center justify-center z-50 transition-all opacity-30 hover:opacity-100 shadow-2xl border border-white/20 pointer-events-auto hover:scale-110"
          style={{ right: Math.max(16, centerOffset - 80) }}
        >
      ...
      >
        ...
        <div 
          className={`flex items-center gap-2 bg-black/60 backdrop-blur-xl p-2 rounded-full border border-white/10 shadow-2xl transition-all duration-500 origin-bottom ${isHoveringBottom ? 'min-w-[500px] opacity-100 scale-100 translate-y-0' : 'min-w-[300px] opacity-40 scale-75 translate-y-2'}`}
        >
          <button 
            onClick={() => setCurrentPage(0)}
            disabled={currentPage === 0}
            className={`p-3 rounded-full text-white transition-all overflow-hidden ${isHoveringBottom ? (currentPage === 0 ? 'opacity-30 w-11 cursor-not-allowed' : 'opacity-100 w-11 hover:bg-white/10') : 'opacity-0 w-0 p-0'}`}
            title="Jump to Start"
          >
          ...
          <button 
            onClick={handlePrev}
            disabled={currentPage === 0}
            className={`p-3 rounded-full text-white transition-all overflow-hidden ${isHoveringBottom ? (currentPage === 0 ? 'opacity-30 w-12 cursor-not-allowed' : 'opacity-100 w-12 hover:bg-white/10') : 'opacity-0 w-0 p-0'}`}
          ...
          <div className="flex-1 flex flex-col justify-center px-4 group py-2">
            <input 
              ...
              className="w-full h-1.5 bg-white/20 rounded-lg appearance-none cursor-pointer accent-[#0057FF] transition-all hover:h-2"
            />
          ...
          <button 
            ...
            className={`p-3 rounded-full text-white transition-all overflow-hidden ${isHoveringBottom ? (currentPage >= totalPages - 1 ? 'opacity-30 w-12 cursor-not-allowed' : 'opacity-100 w-12 hover:bg-white/10') : 'opacity-0 w-0 p-0'}`}
            title="Next Slide"
          >
          ...
          <button 
            ...
            disabled={currentPage >= totalPages - 1}
            className={`p-3 rounded-full text-white transition-all overflow-hidden ${isHoveringBottom ? (currentPage >= totalPages - 1 ? 'opacity-30 w-11 cursor-not-allowed' : 'opacity-100 w-11 hover:bg-white/10') : 'opacity-0 w-0 p-0'}`}
            title="Jump to End"
          >
